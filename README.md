...
| 스킬 | 푸는 문제 | 입력 → 출력 |
|---|---|---|
| [자소서 인터뷰 작성기](skills/cover-letter-builder/SKILL.md) ⭐ | 경험을 **꺼내지 못하고**, 문항이 묻는 축을 **비껴간다** | 문항 → 짧은 인터뷰 → 답안 + 소재 카드 |

문항을 먼저 해부해 필요한 축만 캐므로 인터뷰가 **문항 수에 비례**한다 (3문항 = 10~15분).
캔 재료는 소재 카드로 남아 다음 회사 지원 때 재사용된다.
`career-material-bank` + `cover-letter-writer` 를 하나로 합친 버전.
